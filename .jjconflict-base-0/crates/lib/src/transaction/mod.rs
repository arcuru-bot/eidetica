...
impl Transaction {
    ...
    /// Checks if this transaction is at the current database tips.
    ///
    /// Cache can only be used when transaction is at current tips.
    pub(crate) async fn is_at_current_tips(&self) -> Result<bool> {
        let main_parents = {
            let builder_ref = self.entry_builder.lock().unwrap();
            let builder = builder_ref
                .as_ref()
        ...
        let current_database_tips = self.db.backend()?.get_tips(self.db.root_id()).await?;
        Ok(main_parents == current_database_tips)
    }

    /// Gets the full CRDT state with caching support for Table stores.
    ///
    /// This method checks the cache before computing the full state:
    /// 1. If transaction is not at current tips, skips cache and computes directly
    /// 2. Gets current store tips and cached tips
    /// 3. If tips match, reconstructs Doc from cached rows
    /// 4. If tips don't match, computes full state and rebuilds cache
    ///
    /// # Arguments
    /// * `subtree_name` - The name of the Table store subtree
    ///
    /// # Returns
    /// A `Result<Doc>` containing the merged CRDT state
    pub(crate) async fn get_full_state_cached(&self, subtree_name: &str) -> Result<Doc> {
        ...
        // Check if cache is valid (tips match exactly)
        let cache_valid = match &cached_tips {
            Some(tips) => {
                let mut current_sorted = current_tips.clone();
                let mut cached_sorted = tips.clone();
                current_sorted.sort();
                cached_sorted.sort();
                current_sorted == cached_sorted
            }
            None => false,
        };
    ...
    ///
    /// Cache is valid if:
    /// 1. Transaction is at current tips (not historical)
    /// 2. Cached tips match current store tips exactly
    ///
    /// # Returns
    /// `Ok(true)` if cache is valid and can be used for single-row lookups,
    /// `Ok(false)` if cache needs to be rebuilt.
    pub(crate) async fn is_table_cache_valid(&self, subtree_name: &str) -> Result<bool> {
        // Cache only works at current tips
        if !self.is_at_current_tips().await? {
            return Ok(false);
        }
    ...
    /// Gets a single row from the table cache.
    ///
    /// This does NOT check cache validity - caller must ensure cache is valid first.
    /// Returns `None` if the row doesn't exist in cache.
    ///
    /// # Arguments
    /// * `subtree_name` - The name of the Table store
    /// * `uuid` - The row's UUID key
    ///
    /// # Returns
    /// `Ok(Some(CachedRow))` if found, `Ok(None)` if not in cache.
    pub(crate) async fn get_single_cached_row(
        &self,
        subtree_name: &str,
        uuid: &str,
    ) -> Result<Option<crate::backend::CachedRow>> {
        let backend = self.db.backend()?;
        let tree_id = self.db.root_id();
        backend.get_cached_row(tree_id, subtree_name, uuid).await
    }
    ...
    ///
    /// # Returns
    /// `Ok(true)` if cache is now valid (was valid or rebuilt successfully),
    /// `Ok(false)` if transaction is at historical tips (cache not applicable).
    pub(crate) async fn ensure_table_cache_valid(&self, subtree_name: &str) -> Result<bool> {
        // Cache only works at current tips
        if !self.is_at_current_tips().await? {
            return Ok(false);
        }
