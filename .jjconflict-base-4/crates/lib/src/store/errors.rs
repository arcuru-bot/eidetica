...
pub enum StoreError {
    ...
    /// Attempted to use a store type that has not been registered
    #[error("Store type '{type_id}' is not registered")]
    UnknownStoreType { type_id: String },

    /// Multiple registrations were attempted for the same store type ID
    #[error("Store type '{type_id}' is already registered")]
    RegistrationConflict { type_id: String },
}
