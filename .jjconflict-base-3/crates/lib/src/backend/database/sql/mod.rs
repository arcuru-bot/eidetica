...
/// Default entry cache size limit in bytes (5MB).
const ENTRY_CACHE_SIZE_BYTES: u64 = 5 * 1024 * 1024;

/// Cached entry with pre-computed size for efficient LRU eviction.
#[derive(Clone)]
pub(crate) struct CachedEntry {
    pub entry: Entry,
    /// JSON serialized length, used for size-based cache eviction.
    pub json_len: usize,
}
...
pub struct SqlxBackend {
    pool: AnyPool,
    kind: DbKind,
    /// LRU cache for Entry objects, bounded by total size in bytes.
    entry_cache: Cache<ID, CachedEntry>,
}
...
impl SqlxBackend {
    ...
    pub(crate) fn entry_cache(&self) -> &Cache<ID, CachedEntry> {
        &self.entry_cache
    }

    /// Build the entry cache with size-based eviction.
    fn build_entry_cache() -> Cache<ID, CachedEntry> {
        Cache::builder()
            .max_capacity(ENTRY_CACHE_SIZE_BYTES)
            .weigher(|_key: &ID, value: &CachedEntry| -> u32 {
                // Use JSON length as weight, capped to u32::MAX
                value.json_len.min(u32::MAX as usize) as u32
            })
            .build()
    }
}
...
impl SqlxBackend {
    ...
    pub async fn connect_sqlite(url: &str) -> Result<Self> {
        ...
        let backend = Self {
            pool,
            kind: DbKind::Sqlite,
            entry_cache: Self::build_entry_cache(),
        };
