...
impl Backend {
    ...
    /// Get entries reachable from new_tips but not old_tips (for incremental cache updates)
    pub async fn get_entries_between_tips(
        ...
        self.backend_impl
            .get_entries_between_tips(tree, store, old_tips, new_tips)
            .await
    }
    ...
    /// Get a cached row from the table cache
    pub async fn get_cached_row(
        ...
        self.backend_impl.get_cached_row(tree, store, uuid).await
    }

    /// Upsert a cached row in the table cache
    pub async fn upsert_cached_row(
        &self,
        tree: &ID,
        store: &str,
        uuid: &str,
        row: &crate::backend::CachedRow,
    ) -> Result<()> {
        self.backend_impl
            .upsert_cached_row(tree, store, uuid, row)
            .await
    }

    /// Get all cached rows for a store
    pub async fn get_all_cached_rows(
        &self,
        tree: &ID,
        store: &str,
    ) -> Result<std::collections::HashMap<String, crate::backend::CachedRow>> {
        self.backend_impl.get_all_cached_rows(tree, store).await
    }

    /// Clear all cached rows for a store
    pub async fn clear_cached_rows(&self, tree: &ID, store: &str) -> Result<()> {
        self.backend_impl.clear_cached_rows(tree, store).await
    }

    /// Get cached tips for a store
    pub async fn get_cached_tips(&self, tree: &ID, store: &str) -> Result<Option<Vec<ID>>> {
        self.backend_impl.get_cached_tips(tree, store).await
    }

    /// Set cached tips for a store
    pub async fn set_cached_tips(&self, tree: &ID, store: &str, tips: Vec<ID>) -> Result<()> {
        self.backend_impl.set_cached_tips(tree, store, tips).await
    }
}
