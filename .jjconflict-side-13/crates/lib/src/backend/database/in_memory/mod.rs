...
use crate::{
    Result,
    backend::{BackendImpl, CachedRow, InstanceMetadata, VerificationStatus, errors::BackendError},
    entry::{Entry, ID},
};

/// Row cache for Table stores: store_name -> uuid -> CachedRow
pub(crate) type StoreRowCache = HashMap<String, HashMap<String, CachedRow>>;
...
pub struct InMemory {
    ...
    /// Row cache for Table stores: tree_id -> store_name -> uuid -> CachedRow
    pub(crate) cached_rows: RwLock<HashMap<ID, StoreRowCache>>,
    /// Tips used to compute row cache: tree_id -> store_name -> tips
    pub(crate) cached_tips: RwLock<HashMap<ID, HashMap<String, Vec<ID>>>>,
}
...
impl InMemory {
    ...
    pub fn new() -> Self {
        Self {
            ...
            cached_rows: RwLock::new(HashMap::new()),
            cached_tips: RwLock::new(HashMap::new()),
        }
    }
...
impl BackendImpl for InMemory {
    ...
    async fn get_cached_row(
        ...
        let cached_rows = self.cached_rows.read().await;
        Ok(cached_rows
            .get(tree)
    ...
    async fn upsert_cached_row(
        ...
        let mut cached_rows = self.cached_rows.write().await;
        cached_rows
            .entry(tree.clone())
            .or_default()
            .entry(store.to_string())
            .or_default()
            .insert(uuid.to_string(), row.clone());
        Ok(())
    }
    ...
    async fn clear_cached_rows(&self, tree: &ID, store: &str) -> Result<()> {
        let mut cached_rows = self.cached_rows.write().await;
        if let Some(stores) = cached_rows.get_mut(tree) {
            stores.remove(store);
        }
        Ok(())
    }
    ...
    async fn get_cached_tips(&self, tree: &ID, store: &str) -> Result<Option<Vec<ID>>> {
        let cached_tips = self.cached_tips.read().await;
        Ok(cached_tips
            .get(tree)
            .and_then(|stores| stores.get(store))
            .cloned())
    ...
    async fn set_cached_tips(&self, tree: &ID, store: &str, tips: Vec<ID>) -> Result<()> {
        let mut cached_tips = self.cached_tips.write().await;
        cached_tips
            .entry(tree.clone())
            .or_default()
            .insert(store.to_string(), tips);
        Ok(())
    }
}
