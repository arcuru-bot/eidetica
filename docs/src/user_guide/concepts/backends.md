...
- `current_snapshot(tree_id)`: Returns a `Snapshot` of the latest entries in a specific `Database`.
- `store_snapshot(tree_id, store_name)`: Returns a `Snapshot` of the latest entries _for a specific `Store`_ within a `Database`.
