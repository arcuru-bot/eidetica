...
Eidetica tracks every subtree’s type identifier and configuration inside the `_index` registry. The `StoreRegistry` centralizes constructors for built-in stores (DocStore, Table, YDoc) and any custom stores that embedders register. This enables tooling to enumerate subtrees dynamically without guessing their concrete type.

Use `Database::open_registered_store` for read-only access and `Transaction::open_registered_store` when staging changes:
...
# settings.set("name", "registry_demo");
# let database = user.create_database(settings, &default_key)?;
# let tx = database.new_transaction()?;
# let store = tx.get_store::<DocStore>("data")?;
# store.set("hello", "world")?;
# tx.commit()?;
// Discover type at runtime
let handle = database.open_registered_store("data")?;
println!("{}: {}", handle.subtree(), handle.type_id());
let doc_store: DocStore = handle.downcast()?;

// Use transactional version for writes
let tx = database.new_transaction()?;
let handle = tx.open_registered_store("data")?;
let doc_store: DocStore = handle.downcast()?;
doc_store.set("dynamic", "value")?;
tx.commit()?;
# Ok(())
# }
```

Handles expose `type_id()` and the stored `SubtreeInfo`, allowing tooling to log metadata or dispatch to specific code paths before downcasting. Downcasting converts the handle into a concrete `Store`, returning a `StoreError::TypeMismatch` if the requested type does not match the registry metadata.
