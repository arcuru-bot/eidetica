...
use crate::{
    ...
    store::{BoxedStoreHandle, SettingsStore, Store, StoreRegistry, SubtreeInfo},
};
...
impl Database {
    ...
    pub async fn create(
        ...
        let temp_database_for_bootstrap = Database {
            ...
            store_registry: StoreRegistry::global(),
        };
    ...
    /// Open a store dynamically using the registry metadata stored in `_index`.
    pub fn open_registered_store(&self, name: impl AsRef<str>) -> Result<BoxedStoreHandle> {
        let name = name.as_ref();
        let info = self.lookup_subtree_info(name)?;
        self.store_registry.open_viewer(self, name, &info)
    }
    ...
    pub(crate) fn store_registry(&self) -> StoreRegistry {
        self.store_registry.clone()
    }

    fn lookup_subtree_info(&self, subtree_name: &str) -> Result<SubtreeInfo> {
        let tx = self.new_transaction()?;
        let index = tx.get_index_store()?;
        index.get_subtree_info(subtree_name)
    }
}
