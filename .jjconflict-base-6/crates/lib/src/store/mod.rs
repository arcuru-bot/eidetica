...
mod registry;
pub use registry::{BoxedStoreHandle, StoreRegistration, StoreRegistrationBuilder, StoreRegistry};
...
pub(crate) fn register_builtin_stores(registry: &StoreRegistry) {
    registry
        .register(StoreRegistration::for_store::<DocStore>().build())
        .expect("DocStore registration should succeed");
    registry
        .register(StoreRegistration::for_store::<Table<Value>>().build())
        .expect("Table registration should succeed");

    #[cfg(feature = "y-crdt")]
    registry
        .register(StoreRegistration::for_store::<YDoc>().build())
        .expect("YDoc registration should succeed");
}
