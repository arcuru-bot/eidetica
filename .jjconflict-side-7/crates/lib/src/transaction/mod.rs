...
use crate::{
    ...
    store::{BoxedStoreHandle, IndexStore, SettingsStore, StoreError},
};
...
impl Transaction {
    ...
    /// Open a dynamically registered store within this transaction.
    pub fn open_registered_store(&self, subtree_name: impl AsRef<str>) -> Result<BoxedStoreHandle> {
        let subtree_name = subtree_name.as_ref();
        let index = self.get_index_store()?;
        let info = index.get_subtree_info(subtree_name)?;
        self.db
            .store_registry()
            .open_with_tx(self, subtree_name, &info)
    }
