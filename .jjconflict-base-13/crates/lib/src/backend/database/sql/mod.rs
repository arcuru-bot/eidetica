...
use crate::backend::{BackendImpl, CachedRow, InstanceMetadata, VerificationStatus};
...
impl BackendImpl for SqlxBackend {
    ...
    async fn get_entries_between_tips(
        ...
        traversal::get_entries_between_tips(self, tree, store, old_tips, new_tips).await
    }
    ...
    async fn get_cached_row(
        ...
        storage::get_cached_row(self, tree, store, uuid).await
    }
    ...
    async fn upsert_cached_row(
        ...
        storage::upsert_cached_row(self, tree, store, uuid, row).await
    }
    ...
    async fn get_all_cached_rows(
        ...
        storage::get_all_cached_rows(self, tree, store).await
    }

    async fn clear_cached_rows(&self, tree: &ID, store: &str) -> Result<()> {
        storage::clear_cached_rows(self, tree, store).await
    }

    async fn get_cached_tips(&self, tree: &ID, store: &str) -> Result<Option<Vec<ID>>> {
        storage::get_cached_tips(self, tree, store).await
    }

    async fn set_cached_tips(&self, tree: &ID, store: &str, tips: Vec<ID>) -> Result<()> {
        storage::set_cached_tips(self, tree, store, tips).await
    }
}
