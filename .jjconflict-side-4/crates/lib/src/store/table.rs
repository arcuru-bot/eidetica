    ...
    T: Serialize + for<'de> Deserialize<'de> + Clone,
{
    ...
    /// 1. First checks local staged changes in the transaction
    ...
    /// 4. Falls back to full CRDT computation only for historical transactions
    ///
    /// # Arguments
    ...
    pub async fn get(&self, key: impl AsRef<str>) -> Result<T> {
        ...
        // Fall back to full CRDT computation for historical transactions
