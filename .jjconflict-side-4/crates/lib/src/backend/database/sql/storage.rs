...
use crate::backend::{CachedRow, InstanceMetadata, VerificationStatus};
...
/// Get a cached row from a Table store.
pub async fn get_cached_row(
    backend: &SqlxBackend,
    tree: &ID,
    store: &str,
    uuid: &str,
    ...
    let row: Option<(String, i64, String, i64)> = sqlx::query_as(
        "SELECT data, is_tombstone, last_modified_entry_id, last_modified_height
         FROM cached_rows WHERE tree_id = $1 AND store_name = $2 AND uuid = $3",
    )
    .bind(tree.to_string())
    .bind(store)
    .bind(uuid)
    .fetch_optional(pool)
    .await
    .sql_context("Failed to get cached row")?;

    match row {
        Some((data, is_tombstone, entry_id_str, height)) => {
            let entry_id = ID::from(entry_id_str);
            Ok(Some(CachedRow {
                data,
                is_tombstone: is_tombstone != 0,
                last_modified_entry_id: entry_id,
                last_modified_height: height as usize,
            }))
        }
        None => Ok(None),
    }
...
pub async fn upsert_cached_row(
    backend: &SqlxBackend,
    tree: &ID,
    store: &str,
    uuid: &str,
    ...
    let pool = backend.pool();
    let is_tombstone: i64 = if row.is_tombstone { 1 } else { 0 };

    if backend.is_sqlite() {
        sqlx::query(
            "INSERT OR REPLACE INTO cached_rows
             (tree_id, store_name, uuid, data, is_tombstone, last_modified_entry_id, last_modified_height)
        ...
        .bind(&row.data)
        .bind(is_tombstone)
        .bind(row.last_modified_entry_id.to_string())
        .bind(row.last_modified_height as i64)
        .execute(pool)
        .await
        .sql_context("Failed to upsert cached row")?;
    } else {
        sqlx::query(
            "INSERT INTO cached_rows
             (tree_id, store_name, uuid, data, is_tombstone, last_modified_entry_id, last_modified_height)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             ON CONFLICT (tree_id, store_name, uuid) DO UPDATE SET
             data = EXCLUDED.data,
             is_tombstone = EXCLUDED.is_tombstone,
             last_modified_entry_id = EXCLUDED.last_modified_entry_id,
...
/// Get all cached rows for a Table store.
pub async fn get_all_cached_rows(
    backend: &SqlxBackend,
    tree: &ID,
    store: &str,
    ...
    let rows: Vec<(String, String, i64, String, i64)> = sqlx::query_as(
        "SELECT uuid, data, is_tombstone, last_modified_entry_id, last_modified_height
         FROM cached_rows WHERE tree_id = $1 AND store_name = $2",
    )
    .bind(tree.to_string())
    .bind(store)
    .fetch_all(pool)
    .await
    .sql_context("Failed to get all cached rows")?;

    let mut result = HashMap::new();
    for (uuid, data, is_tombstone, entry_id_str, height) in rows {
        let entry_id = ID::from(entry_id_str);
        result.insert(
            uuid,
            CachedRow {
...
/// Clear all cached rows for a Table store.
pub async fn clear_cached_rows(backend: &SqlxBackend, tree: &ID, store: &str) -> Result<()> {
    let pool = backend.pool();

    sqlx::query("DELETE FROM cached_rows WHERE tree_id = $1 AND store_name = $2")
        .bind(tree.to_string())
        .bind(store)
        .execute(pool)
        .await
        .sql_context("Failed to clear cached rows")?;

    // Also clear the cached tips
    sqlx::query("DELETE FROM cache_tips WHERE tree_id = $1 AND store_name = $2")
        .bind(tree.to_string())
        .bind(store)
        .execute(pool)
        .await
        .sql_context("Failed to clear cache tips")?;

    Ok(())
}

/// Get the tips that the row cache was computed from.
pub async fn get_cached_tips(
    backend: &SqlxBackend,
    tree: &ID,
    store: &str,
    ...
    if rows.is_empty() {
        ...
        let tips: Vec<ID> = rows.into_iter().map(|(tip_id,)| ID::from(tip_id)).collect();
        Ok(Some(tips))
    }
}

/// Set the tips that the row cache was computed from.
pub async fn set_cached_tips(
    backend: &SqlxBackend,
    tree: &ID,
    store: &str,
    tips: Vec<ID>,
    ...
    for tip in tips {
        if backend.is_sqlite() {
            sqlx::query(
                "INSERT OR REPLACE INTO cache_tips (tree_id, store_name, tip_id) VALUES ($1, $2, $3)",
            )
            .bind(tree.to_string())
            .bind(store)
            .bind(tip.to_string())
            .execute(pool)
            .await
            .sql_context("Failed to insert cache tip")?;
        } else {
            sqlx::query(
                "INSERT INTO cache_tips (tree_id, store_name, tip_id) VALUES ($1, $2, $3)
                 ON CONFLICT (tree_id, store_name, tip_id) DO NOTHING",
            )
            .bind(tree.to_string())
            .bind(store)
            .bind(tip.to_string())
