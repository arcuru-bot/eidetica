...
use super::{CachedEntry, SqlxBackend, SqlxResultExt};

/// Get an entry by ID.
pub async fn get(backend: &SqlxBackend, id: &ID) -> Result<Entry> {
    // Check cache first
    if let Some(cached) = backend.entry_cache().get(id).await {
    ...
    match row {
        Some((json,)) => {
            ...
            backend
                .entry_cache()
                .insert(id.clone(), CachedEntry { entry: entry.clone(), json_len })
                .await;
...
pub async fn put(
    ...
    if let Some((existing_status_int,)) = existing_status {
        ...
        // Populate cache (entry content is immutable, so cache is always valid)
        backend
            .entry_cache()
            .insert(id.clone(), CachedEntry { entry, json_len })
            .await;
    ...
    backend
        .entry_cache()
        .insert(id, CachedEntry { entry, json_len })
        .await;
