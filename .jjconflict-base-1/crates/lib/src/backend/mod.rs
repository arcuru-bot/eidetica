...
///
/// This struct represents a materialized view of a single row's state
/// at a specific set of tips. Used by the Table store's row cache
/// for O(1) reads.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub trait BackendImpl: Send + Sync + Any {
    ...
    /// that need to be processed when tips change. The result includes entries that
    /// are ancestors of any new_tip but are not ancestors of any old_tip.
    ///
    /// # Arguments
    /// * `tree` - The root ID of the parent tree.
    /// * `store` - The name of the store to retrieve diffs for.
    /// * `old_tips` - The previous tip IDs (entries reachable from these are excluded).
    /// * `new_tips` - The current tip IDs (entries reachable from these are considered).
    ///
    /// # Returns
    /// A `Result` containing a vector of `Entry` objects that are in the diff,
    /// sorted topologically (by height ascending, then ID).
    ...
    // === Row Cache Methods ===
    //
    // These methods provide caching for Table store rows, enabling O(1) reads
    // when the cache is valid. The cache stores materialized row state
    // along with the tips it was computed from.
    ...
    /// * `store` - The store name
    /// * `uuid` - The row UUID
    /// * `row` - The cached row data
    ///
    /// # Returns
    /// A `Result` indicating success or an error during storage.
    async fn upsert_cached_row(
        &self,
        tree: &ID,
        store: &str,
        uuid: &str,
    ...
    ///
    /// # Returns
    /// A `Result` containing a `HashMap` of UUID to `CachedRow`.
    async fn get_all_cached_rows(
        &self,
        tree: &ID,
        store: &str,
    ...
    /// # Returns
    /// A `Result` indicating success or an error during the clear operation.
    async fn clear_cached_rows(&self, tree: &ID, store: &str) -> Result<()>;
    ...
    ///
    /// # Returns
    /// A `Result` containing an `Option<Vec<ID>>`. Returns `None` if no cache exists.
    async fn get_cached_tips(&self, tree: &ID, store: &str) -> Result<Option<Vec<ID>>>;
    ...
    /// * `tree` - The tree ID
    /// * `store` - The store name
    /// * `tips` - The tip IDs the cache was computed from
    ///
    /// # Returns
    /// A `Result` indicating success or an error during storage.
    async fn set_cached_tips(&self, tree: &ID, store: &str, tips: Vec<ID>) -> Result<()>;
}
