...
async fn test_handshake_automatically_registers_peer() {
    ...
    assert_eq!(*peer_info.id.public_key(), peer_verifying_key);
...
async fn test_duplicate_handshakes_handled_gracefully() {
    ...
    assert_eq!(*peers[0].id.public_key(), peer_verifying_key);
}
...
async fn test_incremental_sync_tracks_tree_peer_relationship() {
    ...
    let tips = instance.backend().current_snapshot(&tree_id).await.unwrap().into_tips();
