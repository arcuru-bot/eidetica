...
pub fn create_entry_with_parents(root: &str, parents: &[&str]) -> Entry {
    Entry::builder(ID::from_bytes(root))
        .set_parents(parents.iter().map(ID::from_bytes).collect())
        .build()
...
pub fn create_entry_with_subtree_parents(
    ...
    Entry::builder(ID::from_bytes(root))
        .add_parent(ID::from_bytes("main_parent")) // Add a main tree parent for valid non-root entry
        .set_subtree_data(subtree_name, data)
        .set_subtree_parents(subtree_name, parents.iter().map(ID::from_bytes).collect())
        .build()
...
pub fn assert_has_parents(entry: &Entry, expected_parents: &[&str]) {
    ...
    for parent in expected_parents {
        assert!(
            parents.contains(&ID::from_bytes(parent)),
...
pub fn create_complex_entry_with_order(root: &str, reverse_order: bool) -> Entry {
    ...
    if reverse_order {
        ...
        builder.set_subtree_parents_mut("sub2", vec![ID::from_bytes("sp3")]);
        builder.set_subtree_parents_mut("sub1", vec![ID::from_bytes("sp2"), ID::from_bytes("sp1")]);
    } else {
        ...
        builder.set_subtree_data_mut("sub2", "data2");
        builder.set_subtree_data_mut("sub3", "data3");
        builder.set_subtree_parents_mut("sub1", vec![ID::from_bytes("sp1"), ID::from_bytes("sp2")]);
        builder.set_subtree_parents_mut("sub2", vec![ID::from_bytes("sp3")]);
    }
...
pub fn create_entry_with_unsorted_parents(
    ...
    builder.set_parents_mut(parents.iter().map(ID::from_bytes).collect());
    ...
    for (subtree_name, subtree_parent_list) in subtree_parents {
        ...
        builder.set_subtree_parents_mut(
            *subtree_name,
            subtree_parent_list.iter().map(ID::from_bytes).collect(),
        );
    }
...
pub fn create_entry_with_duplicate_parents(root: &str, parents_with_dupes: &[&str]) -> Entry {
    Entry::builder(ID::from_bytes(root))
        .set_parents(parents_with_dupes.iter().map(ID::from_bytes).collect())
        .build()
