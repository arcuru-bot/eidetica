...
> tables larger than memory. The rebuild path also has no concurrency
> coordination, allowing thundering herd and redundant work under concurrent
> reads (see [Concurrency Limitations](#concurrency-limitations)). See
> [Table Store: Row-Native Storage](table_storage.md) for the replacement design.
...
The cache has no coordination for concurrent access during rebuilds. These limitations are inherent to the current design and are a primary motivation for the [row-native storage](table_storage.md) replacement.

### Non-Atomic Rebuild

`rebuild_table_cache()` performs three phases as independent backend calls with no surrounding lock or transaction:

```text
1. clear_cached_rows(tree_id, store)        ← deletes all rows (and tips in SQL backend)
2. upsert_cached_row(...) × N               ← inserts new rows one at a time
3. set_cached_tips(tree_id, store, tips)     ← marks cache as valid
```

Between steps 1 and 3, the cache is in a partially-populated state. The SQL backend runs each operation as a standalone query on the connection pool — there is no SQL transaction wrapping the sequence. A crash mid-rebuild leaves partial data, though recovery is safe because `clear_cached_rows` deletes tips first (so the next reader sees no cached tips and triggers a full rebuild).

### TOCTOU Race in ensure_table_cache_valid

`ensure_table_cache_valid()` uses a check-then-act pattern:

```text
1. is_table_cache_valid()  → false (tips mismatch)
2. get_full_state()        → expensive O(n) CRDT computation
3. rebuild_table_cache()   → clear + populate + set tips
```

Two concurrent readers that both observe stale cache at step 1 will both independently compute full CRDT state and both rebuild the cache. This causes:

- **Thundering herd**: Every concurrent reader on a cache miss triggers its own O(n) rebuild
- **Redundant work**: The CRDT computation at step 2 is duplicated across all concurrent rebuilders

### Self-Correcting but Not Race-Free

The tips-based validity check makes the design **self-correcting**: a stale cache is always detected on the next read and triggers a rebuild. Specifically:

- During rebuild (between clear and set-tips), concurrent readers see `cached_tips = None`, detect the mismatch, and trigger their own rebuilds rather than reading partial data.
- If two concurrent rebuilds race and one writes stale tips after the other wrote correct ones, the next reader detects the mismatch and rebuilds again.

However, there is no guarantee against **redundant work** — multiple readers may all rebuild simultaneously after a single write.

### In-Memory Backend: Separate Locks

The in-memory backend stores `cached_rows` and `cached_tips` under separate `RwLock`s. Unlike the SQL backend, `clear_cached_rows` does not clear the cached tips — only `set_cached_tips` updates them. This means:

- Between `clear_cached_rows` and `set_cached_tips`, a concurrent reader could observe empty rows with stale tips still present.
- In practice this is safe because the reader is in a rebuild triggered by a tips mismatch (the stale tips don't match current tips), so the reader would trigger its own rebuild rather than trusting the empty rows.

Additionally, the `cached_rows` and `cached_tips` fields use `std::sync::RwLock` in their declaration but the `BackendImpl` implementation calls `.await` on lock acquisitions, which requires `tokio::sync::RwLock`. This is a type mismatch that needs resolution.

### Impact

For the current single-writer workloads, these limitations are largely theoretical — the self-correcting tips check prevents serving stale data. The primary practical impact is wasted work under concurrent reads after a write. The [row-native storage](table_storage.md) design addresses these issues by moving to incremental cache maintenance and atomic update operations.
