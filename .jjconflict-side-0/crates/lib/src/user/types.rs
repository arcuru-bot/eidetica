...
// ===== Identity Database Types =====

/// Status of a tracked identity database
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
pub enum IdentityStatus {
    /// Identity database is accessible and fully operational
    Active,
    /// Bootstrap request sent but not yet approved; database not yet accessible
    Pending,
}
...
impl FromStr for IdentityStatus {
    ...
    fn from_str(s: &str) -> crate::Result<Self> {
        match s {
            ...
            other => Err(crate::crdt::CRDTError::DeserializationFailed {
                reason: format!("unknown IdentityStatus: {other}"),
            }
            .into()),
        }
    }
...
/// A named identity tracked by a user
///
/// Stored in the user's private database "identities" DocStore.
/// Maps a human-friendly name to the identity database's root ID and current status.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct TrackedIdentity {
    /// Root ID of the identity database (the shareable identity address)
    pub root_id: ID,
...
impl From<TrackedIdentity> for Value {
    fn from(identity: TrackedIdentity) -> Value {
        Value::Doc(Doc::from(identity))
    }
}

impl From<TrackedIdentity> for Doc {
    fn from(identity: TrackedIdentity) -> Doc {
...
impl TryFrom<&Value> for TrackedIdentity {
    type Error = crate::Error;

    fn try_from(value: &Value) -> crate::Result<Self> {
        match value {
            Value::Doc(doc) => TrackedIdentity::try_from(doc),
            _ => Err(crate::crdt::CRDTError::DeserializationFailed {
                reason: "expected Doc value for TrackedIdentity".to_string(),
            }
            .into()),
        }
    }
...
impl TryFrom<&Doc> for TrackedIdentity {
    type Error = crate::Error;

    fn try_from(doc: &Doc) -> crate::Result<Self> {
        let root_id_str = doc.get_as::<&str>("root_id").ok_or_else(|| {
            crate::crdt::CRDTError::ElementNotFound {
                key: "root_id".to_string(),
            }
        })?;
        let root_id = ID::new(root_id_str);

        let status_str = doc.get_as::<&str>("status").ok_or_else(|| {
            crate::crdt::CRDTError::ElementNotFound {
